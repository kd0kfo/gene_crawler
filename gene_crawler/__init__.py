...
def load_keys(_file, key_list):
    """
    Takes a list of keys, searches the given file and returns a dict containing
    each key's value.
    """
...
class Gene():
    ...
    def get_coords(self):
        """
        Parses the location data and returns a tuple containing
        the beginning and end points of the gene
        """
        import re
        if not self.loc:
            return None
        ...
        if len(coords) != 2:
            raise FormatError("Invalid coordinate: '%s'" % self.loc)
        try:
            # GBS files, and therefore self.loc values, are one-indexed.
            # Internally, we want to use zero-indexed values
            return (int(coords[0]) - 1, int(coords[1]) - 1)
...
def str2gene(string):
    """
    Converts a block of gene data from a gbs file and
    converts it to a Gene object
    """
    string = string.strip()
    if not string:
        return None
...
class Assembly():
    ...
    as a tuple containing the key, which is the first 12 characters,
    and a value, which is the remaining characters.
    """
        ...
        with open(self.filename, "r") as infile:
            for line in infile:
                ...
                if key == "gene":  # Start of a gene block
                    if have_gene:
                        # If this is true, two gene blocks are next
                        # to each other. Is this the one we are looking for?
                    ...
                    continue
                if have_gene:
                    # In this case, a gene block just ended,
                    # because another block is starting.
                    if '/gene="%s"' % gene_name in gene_info:
                        return extract_gene(gene_info)
                    have_gene = False
                    gene_info = ""
    ...
    def next(self):
        ...
        Returns they key-value pair for the LINE.
        """
        if self.closed():
            raise StopIteration
...
def write_gene(gene, outfile=None):
    ...
    outfile.write("%s\t(%d,%d)\t\t%s\t%s\t%s\n"
                  % (gene.name, gene_loc[0], gene_loc[1], gene.direction,
                     gene.synonym, ",".join(gene.db_xref)))
